...
int main() {
    ...
    int N = 1023; //并行度设定
    int brin_in = 0;
    int num_steps = 20000 + brin_in; //迭代次数设定
    float adjust_A = 2850;
    float set_time = 180;
    //     N        adjust_A  
    // 3               60
    ...
    std::vector<float> init_net = { 1,2.0,0.5 };  //初始参数设定
    std::vector<float> now_net = { 1,2.0,0.5 };   //记录当前参数
    ...
    strNumber += "_";
    std::string strbeta = "MP_beta0.txt";
    
    std::string strtime = "MP_time.txt";
    ...
    start = clock();  // 开始计时
    // 迭代开始
    for (int i = 0; i < num_steps & time<set_time; ++i) {
