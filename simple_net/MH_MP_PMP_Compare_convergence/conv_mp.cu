...
__global__ void log_likelihood_kernel(float* x, float* y, float* gpu_a, float* nets, int nets_num, int net_size, int data_num, int deep) {
    ...
    if (idx < nets_num) {
        ...
        for (int i = 0; i < data_num; i++) {
            ...
            gpu_a[idx] += (-0.5 * log(2 * MPI * nets[idx * 3 + 2] * nets[idx * 3 + 2]) - 0.5 * temp * temp)/2000;
...
int main() {
    ...
    int device = 1; // 要使用的设备编号，例如第一个GPU设备编号为0
    ...
    std::cout << "it/s:" << CLOCKS_PER_SEC * float(num_steps) / double(end - start) << " all time:" << double(end - start) / CLOCKS_PER_SEC << std::endl;
    for (int i = 0; i < num_steps; i++) {
        for (int j = 0; j < (N + 1); j++) {
