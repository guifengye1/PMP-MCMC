...
__global__ void log_likelihood_kernel(float* x, float* y, float* gpu_a, float* nets, int nets_num, int net_size, int data_num, int deep) {
    ...
    if (idx < nets_num) {
        ...
        for (int d = 0; d < nets_num; d++) {
            if (d != idx) {
                for (int j = 0; j < 3; j++) {
                    double temp = nets[idx * 3 + j] - nets[d * 3 + j];
...
int main() {
    ...
    int N = 511; //并行度设定
    int brin_in = 2000;
    int num_steps =  1000000  + brin_in; //迭代次数设定 1000000 
    float adjust_A = 1450;
    int device = 7; // 要使用的设备编号，例如第一个GPU设备编号为0
    //     N        adjust_A  
    // 3               120
    // 7,15            100
    ...
    const float beta_true = 2.0f;
    const float sigma_true = 0.5f;
    clock_t start, end;
    ...
    float* gpu_x, * gpu_y, * gpu_a, * gpu_nets; //需要传给gpu的数据
    ...
    std::string strbeta = "_MPbeta0.txt";
    std::string strbeta_true = "_MPbeta_true.txt";
    std::string strsigma_true = "_MPsigma_true.txt";
    std::string strA = "_MPA.txt";
    std::string strtime = "_MPtime.txt";
    std::ofstream outbeta(strNumber + strbeta);
    std::ofstream outbeta_true(strNumber + strbeta_true);
    std::ofstream outsigma_true(strNumber + strsigma_true);
    std::ofstream outstrtime(strNumber + strtime);
    std::ofstream outstrA(strNumber + strA);
    ...
    for (int i = 0; i < num_steps; ++i) {
        ...
        for (int index = 1; index < (N + 1); index++) {
            for (int state = 0; state < now_net.size(); state++) {
                nets[index * now_net.size() + state] = nets[state] + updata(gen);
            }
        }
        ...
        log_likelihood_kernel << <gridSize, blockSize >> > (gpu_x, gpu_y, gpu_a, gpu_nets, nets_num, net_size, datas_num, tree_deep);
        //接受率返回cpu  
    ...
    outstrtime << double(end - start) / CLOCKS_PER_SEC;
    std::cout << "it/s:" << CLOCKS_PER_SEC * float(num_steps - brin_in) / double(end - start) << " all time:" << double(end - start) / CLOCKS_PER_SEC << std::endl;
    for (int i = 0; i < num_steps - brin_in; i++) {
        for (int j = 0; j < (N + 1); j++) {
            outbeta << data_log[i * (N + 1) * 3 + j * 3] << "\n";
            outbeta_true << data_log[i * (N + 1) * 3 + j * 3 + 1] << "\n";
            outsigma_true << data_log[i * (N + 1) * 3 + j * 3 + 2] << "\n";
            outstrA << A_log[i * (N + 1) + j] << "\n";
